...
validate_plotted_spec06 "$BENCHMARK"
...
require_var SPEC06_PATH
require_var OUTPUT_ROOT

BENCHMARK_CODE=$(spec06_code "$BENCHMARK") || die "unknown SPEC2006 benchmark '$BENCHMARK'"
BENCHSPEC_DIR=$(resolve_spec06_benchspec)
RUN_DIR=$BENCHSPEC_DIR/$BENCHMARK_CODE/run/${SPEC06_RUN_DIR_NAME:-run_base_ref_Xeon-gcc4.3.0000}
...
CHECKPOINT_CONFIG=${CHECKPOINT_CONFIG:-ooo_8Gmem_10Bn_06}
...
OUTPUT_DIR=$OUTPUT_ROOT/spec06/$CHECKPOINT_CONFIG.MESI_Two_Level/$RUN_CONFIG/$SCHEME/$BENCHMARK/$THREAT_DIR/STT${STT}_Impl${IMP_CHANNEL}/$PRED_DIR
...
export SPEC06_RUN_DIR=$RUN_DIR
export SPEC06_EXEC_PREFIX=${SPEC06_EXEC_PREFIX:-$RUN_DIR}
...
cd "$RUN_DIR"
run_or_print "${GEM5_CMD[@]}" --outdir="$OUTPUT_DIR" "$GEM5_PATH/configs/example/spec06_config.py" "${SIM_OPTS[@]}" 2>&1 | tee -a "$SCRIPT_OUT"
