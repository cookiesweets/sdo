...
#CONFIG_SPARESPEC="UnsafeBaseline"   # Baseline
#"SpareSpec", "SpareSpecLRURandom", "SpareSpecL1ICache", "SpareSpecLRURandomL1ICache"
# CONFIG_SPARESPEC=SpareSpecLRURandom  # SpareSpec
...
case "$THREAT_MODEL" in
  UnsafeBaseline|Spectre|Futuristic) ;;
  *)
    echo "Invalid THREAT_MODEL: $THREAT_MODEL (allowed: UnsafeBaseline, Spectre, Futuristic)"
    exit 1
    ;;
esac
...
MAX_INSTS=100000000
CHECKPOINT_CONFIG="ooo_8Gmem_10Bn_06"     
...
#Need to export SPEC06_PATH
if [ -z ${SPEC06_PATH+x} ];
then
    echo "SPEC06_PATH is unset";
    exit
else
    echo "SPEC06_PATH is set to '$SPEC06_PATH'";
fi
...
# Output Dir
# OUTPUT_DIR=$GEM5_PATH/output/$CHECKPOINT_CONFIG.MESI/$RUN_CONFIG/${CONFIG_SPARESPEC}/$BENCHMARK
OUTPUT_DIR=$GEM5_PATH/output/$CHECKPOINT_CONFIG.MESI/$RUN_CONFIG/${CONFIG_SPARESPEC}/$BENCHMARK/${THREAT_MODEL}/STT${STT}_Impl${IMPLICIT_CHANNEL}
...
#Run Dir
RUN_DIR=$SPEC06_PATH/benchmarks/SPECCPU_2006/benchspec/CPU2006/$BENCHMARK_CODE/run/run_base_ref_Xeon-gcc4.3.0000
...
echo "Command line:"
echo "$0 $*"
echo "================= Hardcoded directories =================="
echo "GEM5_PATH:                                     $GEM5_PATH"
echo "SPEC06_PATH:                                     $SPEC06_PATH"
echo "==================== Script inputs ======================="
echo "BENCHMARK:                                     $BENCHMARK"
echo "SCHEME:                                        $CONFIG_SPARESPEC"
echo "THREAT_MODEL:                                  $THREAT_MODEL"
echo "STT:                                           $STT"
echo "implicit_channel:                              $IMPLICIT_CHANNEL"
...
$GEM5_PATH/build/X86_MESI_Three_Level/gem5.opt \
    ...
    --benchmark=$BENCHMARK --benchmark_stdout=$OUTPUT_DIR/$BENCHMARK.out \
    --benchmark_stderr=$OUTPUT_DIR/$BENCHMARK.err \
    --num-cpus=1 --mem-size=8GB --num-l2caches=1 \
    --checkpoint-dir=$CKPT_OUT_DIR \
    --checkpoint-restore=$INST_TAKE_CHECKPOINT --at-instruction \
    --cpu-type=DerivO3CPU  \
    --num-dirs=1 --ruby --maxinsts=$MAX_INSTS  \
    --network=simple --topology=Mesh_XY --mesh-rows=1 \
    --scheme=SDO --mem_model=TSO \
    --threat_model=$THREAT_MODEL --STT=$STT --impChannel=$IMPLICIT_CHANNEL \
    --ifPrintROB=0  \
